...
AC_schedule = {"170527": [["USA", "FRA"], ["SWE", "JPN"], ["NZL", "FRA"], ["SWE", "GBR"], ["USA", "NZL"], ["JPN", "GBR"],],
     ...
     "170603": [["USA", "NZL"], ["JPN", "GBR"], ["FRA", "SWE"], ["GBR", "USA"]]}
...
def get_recording_times(date):
    ...
    file_names = os.listdir(os.getcwd()+"/{}/data/{}/csv".format(APP_NAME,date))
    s = set([])
    for i in file_names:
...
def get_race_data(date, race_number):
    ...
    script_dir = os.getcwd() +"/{}/data/".format(APP_NAME)
    res = {}
...
def get_plot_data(date, race_number, field, event, interval):
    data = get_race_data(date, race_number)
    competitors = get_competitors(date, race_number)
    times_list = []
    fields_list = []
    ...
    for boat_name in competitors:
        ...
        # normalize times to the start time
        boat_data['Secs'] = boat_data['Secs'] - get_event_time(date, race_number,
                                                               event)
        ...
        times_list.append(boat_data['Secs'])
        fields_list.append(boat_data[field])
        colors_list.append(get_boat_color(boat_name))

    res = {"Boat": competitors, "Secs": times_list, field: fields_list,
           "Colors": colors_list}
    return res
