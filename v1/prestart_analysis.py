...
def get_marks(rr_number, race_number):
    ...
    wind_speed = int(round(np.mean(np.array(execute_query(q))[:,0])))
...
def plot_course(rr_number, race_number, start, end, interval_len):
    ...
    mark_data, wind_speed = get_marks(rr_number, race_number)
    mark_xs = mark_data[:,0]
    mark_ys = mark_data[:,1]
    ...
    plt.title("Round Robin {} Race {} Start Time: {}s to {}s, TWS = {}".format(rr_number, race_number, start, end, wind_speed))
    t = "Trajectories Round Robin {} Race {} {} to {} TWS = {}".format(rr_number, race_number, start, end, wind_speed)
